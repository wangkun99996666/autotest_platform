...
class test_keyword_manage:
    ...
    def show_test_keywords(self, conditionList, valueList, fieldlist, rows):
        ...
        if len(conditionList) and conditionList[0] == 'id':
            ...
            sql = 'select id, keyword, paraCount, template, example,description from test_keyword where status = 1 and keyword like "%' + str(
                valueList[0]) + '%" order by id desc limit ' + str(rows) + ';'
        print(sql)
    ...
    def new_test_keyword(self, name, paraCount, description, template, example):
        sql = "insert into test_keyword (keyword, paraCount, description, template,example) values ('%s','%s','%s','%s','%s');" % (
        name, paraCount, description, template, example)
        print(sql)
    ...
    def copy_test_keyword(self, id):
        ...
        if len(result):
            result = result[0]
            result = self.new_test_keyword(result["keyword"] + '_copy', result["paraCount"], result["description"],
                                           result["template"], result['example'])
        else:
            result = 0
        return result
