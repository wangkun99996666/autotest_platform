...
class test_api_new_manange():
    ...
    def new_test_api(self, product, module, name, url, paras, osign_list, description):
        paras = paras.replace('"', "'")
        sql = string.Template(
            'insert into api_new (product,module,name,url,paras,osign_list,description) values ("$product","$module","$name","$url","$paras","$osign_list","$description");')
        sql = sql.substitute(product=product, module=module, name=name, url=url, paras=paras, osign_list=osign_list,
