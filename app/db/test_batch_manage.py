...
class test_batch_manage(object):
    ...
    def new_test_batch(self, test_suite_id, test_case_id, name, steps, browser_type='Chrome', status='-1'):
        ...
        sql = string.Template(
            'insert into test_batch (test_suite_id, test_case_id, name,steps,browser_type,status) values ("$test_suite_id","$test_case_id","$name","$steps","$browser_type","$status");')
        sql = sql.substitute(test_suite_id=test_suite_id, test_case_id=test_case_id, steps=steps, name=name,
                             browser_type=browser_type, status=status)
    ...
    def rerun_test_batch(self, id, type):
        if type == 'all':
            ...
            sql = 'select id,test_case_id from  test_batch where test_suite_id = %s and status in (-1,2,3,4);' % (
                str(id))
        result = useDB.useDB().search(sql)
        if len(result):
            for case in result:
                ...
                if len(steps):
                    ...
                    steps = re.sub('"', '""', steps)
                    useDB.useDB().insert(
                        'update test_batch set status=0, steps = "%s" where id = %s ;' % (steps, case[0]))
                    log.log().logger.info(
                        'update test_batch set status=0, steps = "%s" where id = %s ;' % (steps, case[0]))
    ...
    def rerun_test_batch_record(self, id, test_case_id):
        ...
        sql = 'update test_batch set status=0, steps =(select steps from test_case where id = %s) where id = %s ;' % (
        test_case_id, id)
        # print(sql)
        useDB.useDB().insert(sql)
        ...
        if len(result):
            for case in result:
                ...
                if len(steps):
                    ...
                    steps = str(steps).replace('\\', '\\\\')
                    sql = string.Template(
                        'update test_batch set status=0, steps = "$steps" where id = $id and ip="$ip"  ;')
                ...
                if len(steps):
                    for browser_type in browser_type_list:
                        if browser_type not in support_browser:
                            ...
                            self.new_test_batch(test_suite_id, test_case_id, steps[0][0], steps[0][1],
                                                browser_type=browser_type, status='0')
                else:
    ...
    def set_test_end(self, result, nowTime, currentStep, screenFileList, id):
        if currentStep == 'package not found':
            ...
            if result == '1':
                ...
                message = currentStep
            sql = 'update test_batch set status = "%s", runtime = "%s", message= "%s", screenshot  = "%s" where id = %s ;' % (
            result, nowTime, message, screenFileList, id)
        useDB.useDB().insert(sql)
