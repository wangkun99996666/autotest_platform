...
function searchTestBatch1(test_suite_id) {
    ...
    $tb_departments.bootstrapTable('refresh', {
        url: '/test_batch.json',
        data: {id: test_suite_id, status: $("#selectStatus1").val(), name: $('#casename1').val(), type: 'test_suite'}
    });
}
...
function return_page(test_batch_id, type) {
    if (type == 'test_case') {
        window.location.href = ('/test_case_runhistory?id=' + test_batch_id);
    } else {
        window.location.href = ('/test_batch_detail?test_suite_id=' + test_batch_id);
    }
